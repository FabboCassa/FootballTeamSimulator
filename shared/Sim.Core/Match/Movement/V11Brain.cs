namespace Sim.Core.Match.Movement
{
    public sealed partial class MatchSimulator
    {
        ...
        /// tasks that follow; until a decision is its own it is v10's. Its set pieces are its own
        /// (<see cref="V11SetPieces"/>), so it is no longer the v10 match draw for draw; the golden
        /// master stays on v10 until v11 replaces it.
        ...
        private sealed class V11Brain : IMatchBrain
        {
            ...
            {
                if (!_setPieces.Act(tick, side, slot)) _v10.Act(tick, side, slot);
            }

            public void Move(int tick, int side, int slot)
            {
                if (!_setPieces.Move(tick, side, slot)) _v10.Move(tick, side, slot);
            }
        }
    }
}
