...
namespace Sim.Core.Tests.Match
{
    ...
    /// default and is what the golden master pins; V11 plays any match, and until it grows its
    /// own decisions it must be V10 to the last draw.
    /// </summary>
    [TestFixture]
    public class MatchBrainTests
    {
        ...
        [TestCase(9, 10, 777UL)]
        public void V11_PlaysAWholeMatch_AndUntilItGrowsItIsTheV10Match(int home, int away, ulong seed)
        {
            ...
            Assert.That(v11.HomeGoals, Is.EqualTo(v10.HomeGoals));
            Assert.That(v11.AwayGoals, Is.EqualTo(v10.AwayGoals));
            Assert.That(MatchReportHasher.Hash(v11), Is.EqualTo(MatchReportHasher.Hash(v10)),
                "V11 delegates to V10, so the whole report — stream included — must be identical.");
        }
