...
- [ ] **0.1 Repo & solution setup** — Monorepo structure per ARCHITECTURE.md §3; `Sim.Core`, `Sim.Core.Tests`, `Contracts` projects; .gitignore (Unity + .NET).
  ✅ `dotnet build` and `dotnet test` succeed from root.
- [ ] **0.2 Unity project** — Unity 6 LTS project in `client/`, UI Toolkit set up, VContainer + UniTask installed, assembly definitions (App/Services/Presenters/Views/MatchView).
  ✅ Empty project opens, enters Play mode with a Boot scene logging "App started".
- [ ] **0.3 Sim.Core → Unity pipeline** — Build script copies `Sim.Core.dll` + `Contracts.dll` into `Assets/Plugins/SimCore/`.
  ✅ A test MonoBehaviour calls a Sim.Core function and logs its result in Unity.
- [ ] **0.4 Deterministic RNG** — PCG32 implementation in `Sim.Core.Random`, seedable, serializable state.
