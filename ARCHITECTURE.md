...
- **Conditional pre-match plans (3.5):** a plan is an ordered list of `MatchRule`s, each a (minute + scoreline) gate plus an action (instruction change and/or substitution). The engine evaluates rules at minute boundaries against the live score (consuming no randomness) and fires each at most once — a fired rule injects exactly the same input a static `MatchInputChange` would, so it leaves the prefix byte-identical and only re-rolls the remainder, and an empty/never-firing ruleset is identical to the legacy engine. Rules are side-agnostic data resolved from a serializable `PrematchPlan` (player ids + enums, like `LineupPlan`/`TacticPlan`); the host stamps the side by which list it passes. This is the headless "skipped/unwatched" execution path and the foundation for online AI-delegation. Fitness-based triggers ("tired player → sub") attach once the condition model lands (4.1).
